...
export async function POST(req: NextRequest) {
  try {
    ...
    const validImages = pages?.filter(p => {
      ...
      return url && !url.includes("Image failed to generate") && url.length > 100;
    }) || [];
    ...
    for (let i = 0; i < (pages || []).length; i++) {
      ...
      if (!url || 
          url.includes("Image failed to generate") || 
          url.length < 100 ||
        ...
        console.log(`Skipping invalid image for page ${i + 1}`);
        // Draw a placeholder rectangle instead
        doc.rect(imageX, imageY, imageW, imageH).fill("#f3f4f6");
        doc.fontSize(12).fillColor("#6b7280").text("No image available", imageX + imageW/2, imageY + imageH/2, { align: "center" });
